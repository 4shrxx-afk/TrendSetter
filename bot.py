...
def embed_mod(action: str, emoji: str, moderator, target, reason: str, color: int, extra: dict = None) -> discord.Embed:
    ...
    e.set_author(name=f"Moderation • {moderator.guild.name}", icon_url=moderator.guild.icon.url if moderator.guild.icon else None)
...
def no_perm(action: str) -> discord.Embed:
    return embed_error("Missing Permissions", f"You need **{action}** permission to use this command.")
...
async def on_member_remove(member: discord.Member):
    ...
    if not ch:
        return
    e = embed_info(f"{E['wave']} Member Left", f"**{member}** (`{member.id}`) has left the server.", color=C_ORANGE)
    e.set_thumbnail(url=member.display_avatar.url)
    await ch.send(embed=e)
...
async def on_message(message: discord.Message):
    ...
    if len(antispam_db[guild_id][user_id]) >= ANTISPAM_LIMIT:
        if not message.author.guild_permissions.manage_messages:
            ...
            e = embed_error("⚡ Anti-Spam Triggered", f"{message.author.mention} was timed out for **5 minutes** for spamming.")
            e.set_footer(text="AutoMod System")
            await message.channel.send(embed=e, delete_after=8)
            log_e = embed_mod("AutoMod: Spam", "⚡", message.guild.me, message.author, "Spamming messages", C_ORANGE)
            await send_log(message.guild, log_e)
...
async def ban(interaction: discord.Interaction, member: discord.Member, reason: str = None, delete_days: int = 1):
    ...
    if member == interaction.user:
        return await interaction.response.send_message(embed=embed_error("Invalid Target", "You cannot ban yourself."), ephemeral=True)
    dm_e = discord.Embed(title=f"{E['ban']} You've been banned from {interaction.guild.name}", color=C_RED)
    ...
    await send_log(interaction.guild, embed_mod("Member Banned", E["ban"], interaction.user, member, reason, C_RED, {"Deleted Messages": f"{delete_days} day(s)"}))
...
async def unban(interaction: discord.Interaction, user_id: str, reason: str = None):
    ...
    try:
        ...
        await interaction.response.send_message(embed=res_e)
        await send_log(interaction.guild, embed_mod("Member Unbanned", E["unban"], interaction.user, user, reason, C_GREEN))
    except (discord.NotFound, ValueError):
        await interaction.response.send_message(embed=embed_error("Not Found", "User not found or not currently banned."), ephemeral=True)
...
async def kick(interaction: discord.Interaction, member: discord.Member, reason: str = None):
    ...
    if member.top_role >= interaction.user.top_role and interaction.user.id not in OWNER_IDS:
        return await interaction.response.send_message(embed=role_too_high(), ephemeral=True)
    dm_e = discord.Embed(title=f"{E['kick']} You've been kicked from {interaction.guild.name}", color=C_ORANGE)
    dm_e.add_field(name="Reason", value=reason or "No reason provided")
...
async def timeout_cmd(interaction: discord.Interaction, member: discord.Member, duration: str, reason: str = None):
    ...
    res_e.add_field(name="Reason",   value=reason or "No reason", inline=True)
    await interaction.response.send_message(embed=res_e)
    dm_e = discord.Embed(title=f"{E['timeout']} You've been timed out in {interaction.guild.name}", color=C_YELLOW)
    dm_e.add_field(name="Duration", value=duration)
    dm_e.add_field(name="Reason",   value=reason or "No reason provided")
    await dm_member(member, dm_e)
    await send_log(interaction.guild, embed_mod("Member Timed Out", E["timeout"], interaction.user, member, reason, C_YELLOW, {"Duration": duration}))
...
async def warn(interaction: discord.Interaction, member: discord.Member, reason: str = None):
    ...
    count = warn_user(str(interaction.guild.id), str(member.id), reason or "No reason provided", str(interaction.user))
    dm_e = discord.Embed(title=f"{E['warn']} You've been warned in {interaction.guild.name}", color=C_YELLOW)
    dm_e.add_field(name="Reason",          value=reason or "No reason provided")
    ...
    res_e.add_field(name="Reason", value=reason or "No reason provided")
    await interaction.response.send_message(embed=res_e)
    await send_log(interaction.guild, embed_mod("Member Warned", E["warn"], interaction.user, member, reason, C_YELLOW, {"Total Warnings": str(count)}))
...
async def purge(interaction: discord.Interaction, amount: int, member: discord.Member = None):
    ...
    log_e = embed_info(f"{E['purge']} Purge", f"**{len(deleted)}** messages deleted in {interaction.channel.mention}{target_str}.\n**By:** {interaction.user.mention}", color=C_BLUE)
...
async def slowmode(interaction: discord.Interaction, seconds: int):
    ...
    if not 0 <= seconds <= 21600:
        return await interaction.response.send_message(embed=embed_error("Invalid Value", "Must be between 0 and 21600 seconds."), ephemeral=True)
...
async def lock(interaction: discord.Interaction, reason: str = None):
    ...
    await interaction.response.send_message(embed=embed_info(f"{E['lock']} Channel Locked", f"**Reason:** {reason or 'No reason provided'}", color=C_RED))
...
async def lockdown(interaction: discord.Interaction, reason: str = None):
    ...
    for ch in interaction.guild.text_channels:
        try:
            ...
            pass
    await interaction.followup.send(embed=embed_info("🚨 SERVER LOCKDOWN", f"**{count}** channels locked.\n**Reason:** {reason or 'No reason'}\n**By:** {interaction.user.mention}", color=C_RED))
...
async def avatar(interaction: discord.Interaction, member: discord.Member = None):
    ...
    e.add_field(name="🔗 Links", value=f"[PNG]({m.display_avatar.with_format('png').url}) | [JPG]({m.display_avatar.with_format('jpg').url}) | [WEBP]({m.display_avatar.with_format('webp').url})")
    await interaction.response.send_message(embed=e)
...
@app_commands.describe(title="Embed title", description="Embed description", color="Hex color e.g. ff0000", channel="Channel to send to")
...
async def help_cmd(interaction: discord.Interaction):
    e = discord.Embed(title=f"{E['bot']} Command List", description="All commands use `/`", color=C_BLUE)
